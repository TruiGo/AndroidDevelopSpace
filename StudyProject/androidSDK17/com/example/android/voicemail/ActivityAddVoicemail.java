...
/**
 * A simple activity that stores user entered voicemail data into voicemail content provider. To be used as a test voicemail source.
 */
public class ActivityAddVoicemail extends Activity {
	private static final Logger logger = Logger.getLogger(ActivityAddVoicemail.class);
	...
	/**
	 * This is created in {@link #onCreate(Bundle)}, and needs to be released in {@link #onDestroy()}.
	 */
	...
	private void storeVoicemail() {
		try {
			Pair<Voicemail, Uri> newVoicemail = new Pair<Voicemail, Uri>(buildVoicemailObjectFromUiElements(), mRecordingUri);
	...
	private Voicemail buildVoicemailObjectFromUiElements() throws ParseException {
		...
		return VoicemailImpl.createForInsertion(time, sender).setDuration(duration).setSourcePackage(sourcePackageName).build();
	}
	...
	/**
	 * An async task that inserts a new voicemail record using a background thread. The tasks accepts a pair of voicemail object and the recording Uri as the param. The result
	 * returned is the error exception, if any, encountered during the operation.
	 */
	private class InsertVoicemailTask extends AsyncTask<Pair<Voicemail, Uri>, Void, Exception> {
		...
		private void insertVoicemail(Voicemail voicemail, Uri recordingUri) throws IOException {
			InputStream inputAudioStream = recordingUri == null ? null : getContentResolver().openInputStream(recordingUri);
			...
			if (inputAudioStream != null) {
				try {
					mVoicemailProviderHelper.setVoicemailContent(newVoicemailUri, inputAudioStream, getContentResolver().getType(recordingUri));
				} finally {
