...
public class RepeatingAlarmFragment extends Fragment {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == R.id.sample_action) {
			...
			// 设置启动重复闹钟(类型,触发时间,重复间隔,挂起Intent)
			...
			AsyncTask<Object, Void, Void> at = new AsyncTask<Object, Void, Void>() {
				@Override
				protected Void doInBackground(Object... params) {
					...
					AlarmManager alarmManager = (AlarmManager) params[0];
					PendingIntent pendingIntent = (PendingIntent) params[1];
					alarmManager.cancel(pendingIntent);
					return null;
				}
			};
			at.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, alarmManager, pendingIntent);
