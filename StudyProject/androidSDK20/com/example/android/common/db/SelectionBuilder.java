 ...
 * 
 * <p>
 * This class provides a convenient frontend for working with SQL. Instead of composing statements manually using string concatenation, method calls are used to construct the
 * statement one clause at a time. These methods can be chained together.
 * 
 * <p>
 ...
 * 
 * <p>
 * In this example, the table name and filters ({@code WHERE} clauses) are both explicitly specified via method call. SelectionBuilder takes care of issuing a "query" command to
 * the database, and returns the resulting {@link Cursor} object.
 * 
 * <p>
 * Inner {@code JOIN}s can be accomplished using the mapToTable() function. The map() function can be used to create new columns based on arbitrary (SQL-based) criteria. In
 * advanced usage, entire subqueries can be passed into the map() function.
 * 
 * <p>
 * Advanced example:
 * 
 ...
 * 
 * <p>
 * In this example, we have two different types of {@code JOIN}s: a left outer join using a modified table name (since this class doesn't directly support these), and an inner join
 * using the mapToTable() function. The map() function is used to insert a count based on specific criteria, executed as a sub-query.
 * 
...
public class SelectionBuilder {
	 ...
	 * 
	 * <p>
	 * User input should never be directly supplied as as part of the selection statement. Instead, use positional parameters in your selection statement, then pass the user input
	 * in via the selectionArgs parameter. This prevents SQL escape characters in user input from causing unwanted side effects. (Failure to follow this convention may have
	 ...
	 *            SQL where statement
	 * @param selectionArgs
	 *            Values to substitute for positional parameters ('?' characters in {@code selection} statement. Will be automatically escaped.
	 * @return Fluent interface
	 */
	 ...
	 * 
	 * <p>
	 * Map columns from a secondary table onto the current result set. References to the column specified in {@code column} will be replaced with {@code table.column} in the SQL
	 * {@code SELECT} clause.
	 * 
	 * @param column
	 ...
	 * 
	 * <p>
	 * In cases where a column is mapped to another data source (either another table, or an SQL sub-query), the column name will be replaced with a more specific, SQL-compatible
	 * representation.
	 * 
	 ...
	 * @param having
	 *            A filter declare which row groups to include in the cursor, if row grouping is being used, formatted as an SQL HAVING clause (excluding the HAVING itself).
