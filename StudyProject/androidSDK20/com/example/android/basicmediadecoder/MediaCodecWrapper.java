...
public class MediaCodecWrapper {
	...
	/**
	 * Callback for decodes frames. Observers can register a listener for optional stream of decoded data
	 */
	...
	/**
	 * Constructs the {@link MediaCodecWrapper} wrapper object around the video codec. The codec is created using the encapsulated information in the {@link MediaFormat} object.
	 * 
	 * @param trackFormat
	 ...
	 * 
	 * A "sample" here refers to a single atomic access unit in the media stream. The definition of "access unit" is dependent on the type of encoding used, but it typically refers
	 * to a single frame of video or a few seconds of audio. {@link android.media.MediaExtractor} extracts data from a stream one sample at a time.
	 * 
	 * @param input
	 *            A ByteBuffer containing the input data for one sample. The buffer must be set up for reading, with its position set to the beginning of the sample data and its
	 *            limit set to the end of the sample data.
	 * 
	 * @param presentationTimeUs
	 *            The time, relative to the beginning of the media stream, at which this buffer should be rendered.
	 * 
	 * @param flags
	 *            Flags to pass to the decoder. See {@link MediaCodec#queueInputBuffer(int, int, int, long, int)}
	 * 
	 * @throws MediaCodec.CryptoException
	 */
	public boolean writeSample(final ByteBuffer input, final MediaCodec.CryptoInfo crypto, final long presentationTimeUs, final int flags)
		...
		if (size > 0 && !mAvailableInputBuffers.isEmpty()) {
			...
			if (size > buffer.capacity()) {
				throw new MediaCodecWrapper.WriteException(String.format("Insufficient capacity in MediaCodec buffer: "
						+ "tried to write %d, buffer capacity is %d.", input.remaining(), buffer.capacity()));
			}
	 ...
	 * 
	 * @param extractor
	 *            Instance of {@link android.media.MediaExtractor} wrapping the media.
	 * 
	 * @param presentationTimeUs
	 *            The time, relative to the beginning of the media stream, at which this buffer should be rendered.
	 * 
	 ...
	 * @throws MediaCodec.CryptoException
	 */
	public boolean writeSample(final MediaExtractor extractor, final boolean isSecure, final long presentationTimeUs, int flags) {
		boolean result = false;
		boolean isEos = false;
	...
	/**
	 * Processes, releases and optionally renders the output buffer available at the head of the queue. All observers are notified with a callback. See
	 * {@link OutputSampleListener#outputSample(MediaCodecWrapper, android.media.MediaCodec.BufferInfo, java.nio.ByteBuffer)}
	 * 
	 * @param render
	 *            True, if the buffer is to be rendered on the {@link Surface} configured
	 * 
	 */
			...
			case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
				if (mOutputFormatChangedListener != null) {
					mHandler.post(new Runnable() {
						@Override
						public void run() {
							mOutputFormatChangedListener.outputFormatChanged(MediaCodecWrapper.this, mDecoder.getOutputFormat());
