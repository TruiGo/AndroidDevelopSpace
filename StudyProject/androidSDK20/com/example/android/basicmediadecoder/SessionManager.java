...
/**
 * SessionManager manages a media session as a queue. It supports common queuing behaviors such as enqueue/remove of media items, pause/resume/stop, etc.
 * 
 * Actual playback of a single media item is abstracted into a Player interface, and is handled outside this class.
 */
public class SessionManager implements Player.Callback {
	...
	public PlaylistItem add(Uri uri, String mime, PendingIntent receiver) {
		...
		PlaylistItem item = new PlaylistItem(Integer.toString(mSessionId), Integer.toString(mItemId), uri, mime, receiver);
		mPlaylist.add(item);
		mItemId++;
	...
	public PlaylistItem seek(String iid, long pos) {
		...
		if (pos != item.getPosition()) {
			...
			if (item.getState() == MediaItemStatus.PLAYBACK_STATE_PLAYING || item.getState() == MediaItemStatus.PLAYBACK_STATE_PAUSED) {
				mPlayer.seek(item);
			}
		}
		return item;
	...
	public MediaSessionStatus getSessionStatus(String sid) {
		int sessionState = (sid != null && sid.equals(mSessionId)) ? MediaSessionStatus.SESSION_STATE_ACTIVE
	...
	private void updatePlaybackState() {
		...
		if (item != null) {
			if (item.getState() == MediaItemStatus.PLAYBACK_STATE_PENDING) {
				item.setState(mPaused ? MediaItemStatus.PLAYBACK_STATE_PAUSED : MediaItemStatus.PLAYBACK_STATE_PLAYING);
				if (!mPlayer.isQueuingSupported()) {
					mPlayer.play(item);
				}
	...
	private PlaylistItem removeItem(String iid, int state) {
		checkPlayerAndSession();
		List<PlaylistItem> queue = new ArrayList<PlaylistItem>(mPlaylist.size());
		PlaylistItem found = null;
		for (PlaylistItem item : mPlaylist) {
			if (iid.equals(item.getItemId())) {
				if (mPlayer.isQueuingSupported()) {
					mPlayer.remove(item.getRemoteItemId());
				} else if (item.getState() == MediaItemStatus.PLAYBACK_STATE_PLAYING || item.getState() == MediaItemStatus.PLAYBACK_STATE_PAUSED) {
					mPlayer.stop();
				}
				item.setState(state);
				found = item;
	...
	private void finishItem(boolean error) {
		PlaylistItem item = getCurrentItem();
		if (item != null) {
			removeItem(item.getItemId(), error ? MediaItemStatus.PLAYBACK_STATE_ERROR : MediaItemStatus.PLAYBACK_STATE_FINISHED);
			updateStatus();
		}
	}
