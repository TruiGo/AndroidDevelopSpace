 ...
 * 
 * This class is used as a backend by PlaybackManager to feed media items to the remote route. When the remote route doesn't support queuing, media items are fed one-at-a-time;
 * otherwise media items are enqueued to the remote side.
 */
public class RemotePlayer extends Player {
	...
	private StatusCallback mStatusCallback = new StatusCallback() {
		...
		public void onItemStatusChanged(Bundle data, String sessionId, MediaSessionStatus sessionStatus, String itemId, MediaItemStatus itemStatus) {
			...
			if (mCallback != null) {
				if (itemStatus.getPlaybackState() == MediaItemStatus.PLAYBACK_STATE_FINISHED) {
					mCallback.onCompletion();
				} else if (itemStatus.getPlaybackState() == MediaItemStatus.PLAYBACK_STATE_ERROR) {
					mCallback.onError();
				}
			}
		}
	...
	public void connect(RouteInfo route) {
		...
		if (DEBUG) {
			Log.d(TAG, "connected to: " + route + ", isRemotePlaybackSupported: " + mClient.isRemotePlaybackSupported() + ", isQueuingSupported: "
					+ mClient.isQueuingSupported());
		}
	}
		...
		if (mRoute != null && mRoute.supportsControlRequest(intent)) {
			ControlRequestCallback callback = new ControlRequestCallback() {
				...
				public void onResult(Bundle data) {
					...
					if (data != null) {
						int playbackCount = data.getInt(SampleMediaRouteProvider.DATA_PLAYBACK_COUNT, -1);
	...
	private void logStatus(String message, String sessionId, MediaSessionStatus sessionStatus, String itemId, MediaItemStatus itemStatus) {
		if (DEBUG) {
			String result = "";
			...
			if (itemId != null & itemStatus != null) {
				result += (result.isEmpty() ? "" : ", ") + "itemId=" + itemId + ", itemStatus=" + itemStatus;
			}
