 ...
 * </p>
 * 
 * Each row is then populated with columns that are also defined as XPath-like projections. These projections fetch attributes values or text in the matching row node or its
 * children.
 * 
 * <p>
 ...
 * </p>
 * 
 * <h2>Node selection syntax</h2> The node selection syntax is made of the concatenation of an arbitrary number (at least one) of <code>/node_name</code> node selection patterns.
 * 
 * <p>
 * The <code>/root/child1/child2</code> pattern will for instance match all nodes named <code>child2</code> which are children of a node named <code>child1</code> which are
 * themselves children of a root node named <code>root</code>.
 * </p>
 * 
 * Any <code>/</code> separator in the previous expression can be replaced by a <code>//</code> separator instead, which indicated a <i>descendant</i> instead of a child.
 * 
 * <p>
 * The <code>//node1//node2</code> pattern will for instance match all nodes named <code>node2</code> which are descendant of a node named <code>node1</code> located anywhere in
 * the document hierarchy.
 * </p>
 * 
 * Node names can contain namespaces in the form <code>namespace:node</code>.
 * 
 * <h2>Projection syntax</h2> For every selected node, the projection will then extract actual data from this node and its descendant.
 * 
 * <p>
 * Use a syntax similar to the selection syntax described above to select the text associated with a child of the selected node. The implicit root of this projection pattern is the
 * selected node. <code>/</code> will hence refer to the text of the selected node, while <code>/child1</code> will fetch the text of its child named <code>child1</code> and
 * <code>//child1</code> will match any <i>descendant</i> named <code>child1</code>. If several nodes match the projection pattern, their texts are appended as a result.
 * </p>
 * 
 * A projection can also fetch any node attribute by appending a <code>@attribute_name</code> pattern to the previously described syntax. <code>//child1@price</code> will for
 * instance match the attribute <code>price</code> of any <code>child1</code> descendant.
 * 
 * <p>
 * If a projection does not match any node/attribute, its associated value will be an empty string.
 * </p>
 * 
 * <h2>Example</h2> Using the following XML document:
 * 
 * <pre class="prettyprint">
 * &lt;library&gt;
 *   &lt;book id="EH94"&gt;
 ...
 * </pre>
 * 
 * A selection pattern of <code>/library//book</code> will match the three book entries (while <code>/library/book</code> will only match the first two ones).
 * 
 * <p>
 * Defining the projections as <code>/title</code>, <code>/author</code> and <code>@id</code> will retrieve the associated data. Note that the author of the second book as well as
 * the id of the third are empty strings.
 */
public class XmlDocumentProvider extends ContentProvider {
	/*
	 * Ideas for improvement: - Expand XPath-like syntax to allow for [nb] child number selector - Address the starting . bug in AbstractCursor which prevents a true XPath syntax.
	 * - Provide an alternative to concatenation when several node match (list-like). - Support namespaces in attribute names. - Incremental Cursor creation, pagination
	 */
	 ...
	 * 
	 * <p>
	 * The XML document can be a local resource or a file that will be downloaded from the Internet. In the latter case, your application needs to request the INTERNET permission
	 * in its manifest.
	 * </p>
	 * 
	 * The URI will be of the form <code>content://xmldocument/?resource=R.xml.myFile</code> for a local resource. <code>xmldocument</code> should match the authority declared for
	 * this provider in your manifest. Internet documents are referenced using <code>content://xmldocument/?url=</code> followed by an encoded version of the URL of your document
	 * (see {@link Uri#encode(String)}).
	 * 
	 * <p>
	 * The number of columns of the resulting Cursor is equal to the size of the projection array plus one, named <code>_id</code> which will contain a unique row id (allowing the
	 * Cursor to be used with a {@link CursorAdapter}). The other columns' names are the projection patterns.
	 * </p>
	 * 
	 * @param uri
	 *            The URI of your local resource or Internet document.
	 * @param projection
	 *            A set of patterns that will be used to extract data from each selected node. See class documentation for pattern syntax.
	 * @param selection
	 *            A selection pattern which will select the nodes that will create the Cursor's rows. See class documentation for pattern syntax.
	 * @param selectionArgs
	 *            This parameter is ignored.
	 * @param sortOrder
	 *            The row order in the resulting cursor is determined from the node order in the XML document. This parameter is ignored.
	...
	/**
	 * Creates an XmlPullParser for the provided local resource. Can be overloaded to provide your own parser.
	 * 
	 * @param resourceUri
