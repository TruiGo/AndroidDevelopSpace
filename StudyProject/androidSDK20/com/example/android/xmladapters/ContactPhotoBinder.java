...
/**
 * This custom cursor binder is used by the adapter defined in res/xml to bind contacts photos to their respective list item. This binder simply queries a contact's photo based on
 * the contact's id and sets the photo as a compound drawable on the TextView used to display the contact's name.
 */
public class ContactPhotoBinder extends Adapters.CursorBinder {
	...
	public boolean bind(View view, Cursor cursor, int columnIndex) {
		...
		if (d == null) {
			...
			InputStream stream = ContactsContract.Contacts.openContactPhotoInputStream(mContext.getContentResolver(), uri);
