...
public class WeatherWidgetProvider extends AppWidgetProvider {
			...
			sWorkerQueue.post(new Runnable() {
				...
				public void run() {
					...
					for (int i = 0; i < count; ++i) {
						...
						values.put(WeatherDataProvider.Columns.TEMPERATURE, new Random().nextInt(sMaxDegrees));
	...
	private RemoteViews buildLayout(Context context, int appWidgetId, boolean largeLayout) {
		...
		if (largeLayout) {
			...
			Cursor c = context.getContentResolver().query(WeatherDataProvider.CONTENT_URI, null, null, null, null);
