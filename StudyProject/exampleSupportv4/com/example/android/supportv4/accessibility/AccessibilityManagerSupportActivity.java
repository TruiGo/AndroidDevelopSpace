...
/**
 * <p>
 * This class demonstrates how to use the support library to register an
 * AccessibilityManager.AccessibilityStateChangeListener introduced in ICS to
 * watch changes to the global accessibility state on the device in a backwards
 * compatible manner.
 * </p>
 * <p>
 * This class also demonstrates how to use the support library to query
 * information about enabled accessibility services via APIs introduced in ICS
...
public class AccessibilityManagerSupportActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mAccessibilityManager = (AccessibilityManager) getSystemService(Service.ACCESSIBILITY_SERVICE);
	...
	/**
	 * Registers an AccessibilityStateChangeListener that show a Toast when the
	 * global accessibility state on the device changes.
	 */
	private void registerAccessibilityStateChangeListener() {
		// The AccessibilityStateChange listener APIs were added in ICS.
		// Therefore to be
		// backwards compatible we use the APIs in the support library. Note
		// that if the
		// platform API version is lower and the called API is not available no
		// listener
		// is added and you will not receive a call of
	...
	private void updateAccessibilityStateView() {
		// The API for getting the enabled accessibility services based on
		// feedback
		// type was added in ICS. Therefore to be backwards compatible we use
		// the
		// APIs in the support library. Note that if the platform API version is
		// lower
		// and the called API is not available an empty list of services is
		// returned.
		List<AccessibilityServiceInfo> enabledServices = AccessibilityManagerCompat.getEnabledAccessibilityServiceList(
				mAccessibilityManager, AccessibilityServiceInfo.FEEDBACK_SPOKEN);
		if (!enabledServices.isEmpty()) {
			...
			for (int i = 0; i < enabledServiceCount; i++) {
				...
				// Some new APIs were added in ICS for getting more information
				// about
				// an accessibility service. Again accessed them via the support
				// library.
				ResolveInfo resolveInfo = AccessibilityServiceInfoCompat.getResolveInfo(service);
				String serviceDescription = getString(R.string.accessibility_manager_enabled_service,
			...
			// Either no services or the platform API version is not high
			// enough.
